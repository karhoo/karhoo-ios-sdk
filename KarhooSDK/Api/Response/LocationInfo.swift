...
public struct LocationInfo: KarhooCodableModel {
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.poiType = (try? container.decode(PoiType.self, forKey: .poiType)) ?? .notSetPoiType
        self.address = (try? container.decode(LocationInfoAddress.self, forKey: .address)) ?? LocationInfoAddress()
        self.timeZoneIdentifier = (try? container.decode(String.self, forKey: .timeZoneIdentifier)) ?? ""
        self.details = (try? container.decode(PoiDetails.self, forKey: .details)) ?? PoiDetails()
        self.meetingPoint = (try? container.decode(MeetingPoint.self, forKey: .meetingPoint)) ?? MeetingPoint()
